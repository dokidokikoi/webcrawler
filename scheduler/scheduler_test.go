...
func TestSchedSimple(t *testing.T) {
	...
	for _ = range tickCh {
		if sched.Idle() {
			count++
			log.L().Sugar().Infof("Increase idle count, and value is %d.", count)
		}
		if count >= max {
			log.L().Sugar().Infof("The idle count is equal or greater than %d.", max)
			break
		}
	}
	if err = sched.Stop(); err != nil {
	...
	log.L().Sugar().Infof("-- Final summary:\n%s", sched.Summary())
}
